...
const int rightButton = 11;   // input pin for right click
const int xAxis  = 0;         // joystick X axis to A1 
const int yAxis  = 1;         // joystick Y axis to A0
const int ledPin = 13;        // Mouse control LED 

// parameters for reading the joystick
int range = 20;               // output range of X or Y movement (zero to range)
...
boolean rightButtonPressed = false; // whether or not right button pressed
int  lastMouseReading = 1;       // last joystick/mouse button reading
int  lastRightReading = 1;       // last right button reading
...
void setup() {
  ...
  pinMode(rightButton, INPUT_PULLUP);   // mouse button on joystick
...
//      digitalWrite(ledPin, mouseIsActive);   // toggle LED to indicate mouse state
  ...
  buttonReading = digitalRead(mouseButton);  // read the mouse left button
  
  if(buttonReading != lastMouseReading) {         // switch changed
  ...
  lastMouseReading = buttonReading;

  buttonReading = digitalRead(rightButton);  // read the right button
  
  if(buttonReading != lastRightReading) {         // switch changed
     debounceTime = millis();                // reset debounce timer
  }
  if((millis() - debounceTime) > debounce) {
     buttonState = buttonReading;
  ...
  lastRightReading = buttonReading;
  digitalWrite(ledPin, mouseButtonPressed);  // toggle LED to indicate button state
  
  // if the mouse control state is active, move the mouse:
  if (mouseIsActive) {
      if (rightButtonPressed) {
         TrinketMouse.move(xReading, yReading, 0, MOUSEBTN_RIGHT_MASK);
      } 
      else if (mouseButtonPressed) {  // if joystick pressed down, indicate that too
